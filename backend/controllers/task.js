...
exports.getTasksDeadline = (req, res, next) => {
    db.all('select task.id, task.list_id, task.description, task.title, task.deadline, task.completed from task inner join list on list_id = list.id where user_id = ? and deadline != "" order by 5 asc', [req.body.user_id], (err,data) => {
        if(err){
            return console.error(err.message);
        }
        else{
